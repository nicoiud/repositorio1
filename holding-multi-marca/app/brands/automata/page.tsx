...
export default function AutomataAILanding() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) router.push('/gracias?marca=automata');
    } catch {
      ...
      <section id="contacto" className="bg-gradient-to-r from-violet-600 to-violet-700 text-white py-20 px-4">
        <div className="max-w-2xl mx-auto">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <button type="submit" disabled={loading} className="w-full bg-white text-violet-600 font-bold py-3 rounded-lg hover:bg-gray-100 transition-colors disabled:opacity-70 flex items-center justify-center gap-2">
                {loading ? <><Loader2 size={18} className="animate-spin" /> Enviando...</> : 'Solicitar Demo Gratis'}
              </button>
            </form>
